...
STATIC unsigned  int
S_precedence(const U8 my_operator) {
                    ...
                    char * const oregcomp_parse)
{
    ...
    AV* stack;                      /* stack of operators and operands not yet
                                       resolved */
    AV* fence_stack = NULL;         /* Where would the parentheses be in the
                                       normal stack if they were actually put
                                       there */
...
#define IS_OPERAND(a)  (! SvIOK(a))
     ...
     * replace special tests for parens with subtraction of the fence
     ...
     *
     * The only unary operator '!' is pushed onto the stack.  When an operand
     * is encountered, if the top of the stack is a '!", the complement is
     * immediately performed and the '!' popped.  The resulting value is
     * treated as a new operand, and the logic in the previous paragraph is
     * executed.  Thus in the expression
     *      [a] + ! [b]
     ...
     *
     * The ']' ending the construct is the treated as the lowest operator of
     * all, so that everything gets evaluated down to a single operand, which
     * is the result */
    ...
    while (RExC_parse < RExC_end) {
        I32 top_index;
        SV** top_ptr;               /* Pointer top main stack element */
        SV* current = NULL;
      ...
      redo_curchar:
        ...
        switch (curchar) {
            SV** stacked_ptr;       /* Ptr to something already on the main
                                       stack */
            char stacked_operator;  /* The topmost operator on the main stack.
                                     */
            ...
            case '^':
             ...
             join_operators:
                    ...
                    || IS_OPERAND(*stacked_ptr))
                {
                     ...
                     * done if we're currently processing a ']', which should
                     * be the final thing in the expression */
                    if (curchar == ']') {
                        goto done;
                    }
                ...
                if (precedence(curchar) > precedence(stacked_operator)) {
