...
sub _meta_notation ($) {
    ...
    # into '\x{...}'  Meta notation is meaningless on EBCDIC platforms.

    my $input = shift;

    $input =~ s/([\0-\037])/sprintf("^%c",utf8::unicode_to_native(utf8::native_to_unicode(ord($1))^64))/eg;
    $input =~ s/\c?/^?/g;
    if (ord("A") == 65) {
        $input =~ s/([\200-\377])/sprintf("M-%c",ord($1)&0177)/eg;
    }
    else {
        no warnings 'experimental::regex_sets';
        $input =~ s/(   # Leave alone things above \xff
                      (?[ [\x00-\xFF] & [:^print:]])
